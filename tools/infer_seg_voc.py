...
parser.add_argument("--mil", default=True, type=bool, help="w_mygo")
...
parser.add_argument("--scales", nargs='+', type=float, default=[1.0, 1.25,1.5], help="multi_scales for seg")
...
def _validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            # seg = torch.max(seg_stack[:],dim=0)[0]
            seg = torch.max(torch.max(seg_stack[:1],dim=0)[0],seg_stack[0:].mean(dim = 0))
            # # print(seg.shape)
            # resized_segs = F.interpolate(segs, size=labels.shape[1:], mode='bilinear', align_corners=False)
            seg_pred += list(torch.argmax(seg, dim=1).cpu().numpy().astype(np.int16))
            gts += list(labels.cpu().numpy().astype(np.int16))
            single_ans = evaluate.scores(labels.cpu().numpy().astype(np.int16), torch.argmax(seg, dim=1).cpu().numpy().astype(np.int16))
            save_json[name[0]] = single_ans['iou']
            # print(seg.shape)
