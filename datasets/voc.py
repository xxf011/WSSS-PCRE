...
class VOC12ClsDataset(VOC12Dataset):
    ...
    def __transforms(self, image,seg_mask):
        ...
        if self.aug:
            ...
            if self.crop_size:
                image, seg_mask,img_box = transforms.random_crop(image, label=seg_mask, crop_size=self.crop_size, mean_rgb=[0,0,0], ignore_index=self.ignore_index)
        ...
        return image, local_image, img_box,seg_mask
    ...
    def __getitem__(self, idx):
        ...
        img_name, image, seg_mask = super().__getitem__(idx)
        
        pil_image = Image.fromarray(image)

        image, local_image, img_box,seg_mask = self.__transforms(image=image,seg_mask=seg_mask)
        ...
        if self.aug:
            ...
            return img_name, image, cls_label,label_idx, img_box, raw_image, w_image, s_image,seg_mask
        else:
