...
def cal_eta(time0,time1, cur_iter, total_iter, log_iters):
    time_now = datetime.datetime.now()
    time_now = time_now.replace(microsecond=0)

    # 剩余迭代数
    remaining_iters = total_iter - cur_iter
    

    # 计算已经过的时间
    delta = time_now - time1
    
    # 估计完成剩余部分的时间
    scale = remaining_iters / (log_iters + 1e-6)
    eta = delta * scale
    ...
    return str(time_now - time0), str(eta)
