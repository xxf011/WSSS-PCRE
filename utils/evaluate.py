...
def scores(label_trues, label_preds, num_classes=21):
    ...
    TP = np.diag(hist)
    FN = hist.sum(axis=1) - TP ## hist.sum(axis=1) : label 里的阳
    FP = hist.sum(axis=0) - TP ## hist.sum(axis=0)   模型预测的阳

    cr = FP / TP
    cr = dict(zip(range(num_classes), cr))
...
def incremental_scores(hist, label_true, label_pred, num_classes):
    # 逐步更新混淆矩阵
    hist += _fast_hist(label_true.flatten(), label_pred.flatten(), num_classes)
    return hist

def compute_final_scores(hist, num_classes=21):
    np.seterr(divide='ignore', invalid='ignore')
