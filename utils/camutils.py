import time
import numpy as np
import torch
import torch.nn.functional as F

from model.losses import ClsTokenLoss

def assign_csc_tags(images, roi_mask=None, crop_num=8, crop_size=96, threshold=0.2):

    crops = []
    b, c, h, w = images.shape

    temp_crops = torch.zeros(size=(b, crop_num, c, crop_size, crop_size)).to(images.device)
    flags = torch.ones(size=(b, crop_num+2)).to(images.device)
    margin = crop_size//2

    mask_255 = roi_mask != 255
    roi_mask_ = roi_mask * mask_255 # remove 255
    mask_m1 = roi_mask_ != -1
    roi_mask = roi_mask_ * mask_m1 # remove255 & -1
    roi_mask_un = roi_mask + (roi_mask_ == -1) * 1

    for i1 in range(b):
        roi_index = (roi_mask_[i1, margin:(h-margin), margin:(w-margin)] < 0).nonzero()
        if roi_index.shape[0] < crop_num:
            roi_index = (roi_mask[i1, margin:(h-margin), margin:(w-margin)] >= 0).nonzero() ## if NULL then random crop
        torch.manual_seed(0)

        # multi-cls to -1  
        cls_mask = torch.unique(roi_mask[i1])
        cls_mask = cls_mask[cls_mask != 0]
        if cls_mask.size()[-1] >=2 :
            flags[i1,:2] = -1
        # single-cls
        elif  cls_mask.size()[-1] == 1:
            cls_index = cls_mask[0]
            flags[i1,:2] = cls_index
            roi_mask[i1] = roi_mask[i1] + (roi_mask_[i1] == -1) * cls_index

        rand_index = torch.randperm(roi_index.shape[0])
        crop_index = roi_index[rand_index[:crop_num], :]
        
        for i2 in range(crop_num):
            h0, w0 = crop_index[i2, 0], crop_index[i2, 1] # centered at (h0, w0)
            temp_crops[i1, i2, ...] = images[i1, :, h0:(h0+crop_size), w0:(w0+crop_size)]
            temp_mask_un = roi_mask_un[i1, h0:(h0+crop_size), w0:(w0+crop_size)]
            temp_mask = roi_mask[i1, h0:(h0+crop_size), w0:(w0+crop_size)]

            if temp_mask_un.sum() / (crop_size*crop_size) <= threshold: # 搞笑，那我80 类不是更吊
                flags[i1, i2+2] = 0
            else: 
                temp_mask_cls = torch.unique(temp_mask)
                temp_mask_cls = temp_mask_cls[temp_mask_cls != 0]
                if temp_mask_cls.size()[-1] == 1: # bkg and one cla
                # if temp_mask_cls.size()[-1] == 2 and temp_mask_cls[0] == 0: # bkg and one cla
                    flags[i1,i2+2] = temp_mask_cls[-1]
                else:
                    # bkg and 2 and more cls
                    flags[i1,i2+2] = -1

    _crops = torch.chunk(temp_crops, chunks=crop_num, dim=1,)
    crops = [c[:, 0] for c in _crops]

    return crops, flags


def cam_to_label(cam, cls_label, img_box=None, bkg_thre=None, high_thre=None, low_thre=None, ignore_mid=False, ignore_index=None, auxiliaryMasks=None,ret=False,seg_ts = 0.3):
    b, c, h, w = cam.shape
    #pseudo_label = torch.zeros((b,h,w))
    cls_label_rep = cls_label.unsqueeze(-1).unsqueeze(-1).repeat([1,1,h,w])
    valid_cam = cls_label_rep * cam
    if auxiliaryMasks is not None:
        # valid_cam = (0.7 * valid_cam + 0.3 * auxiliaryMask)
        for auxiliaryMask,seg_t in zip(auxiliaryMasks,seg_ts):
            if auxiliaryMask is None:
                continue
            for idx,(b,cls_idx) in enumerate(cls_label.nonzero()):
                if auxiliaryMask[idx] is not None:
                    if auxiliaryMask[idx].sum() <= 1e-5:
                        continue
                    mask = valid_cam[b, cls_idx] >= 0
                    if auxiliaryMask.shape[-2:] != valid_cam[b, cls_idx].shape:
                        aux_mask = F.interpolate(auxiliaryMask[idx].unsqueeze(0),valid_cam[b, cls_idx].shape)
                        if isinstance(seg_t, float):
                            valid_cam[b, cls_idx] = seg_t * aux_mask[0,0] + (1-seg_t) * valid_cam[b, cls_idx]
                        elif isinstance(seg_t, str):
                            if seg_t == 'max':
                                valid_cam[b, cls_idx] = torch.max(aux_mask[0, 0], valid_cam[b, cls_idx])
                            elif seg_t == 'x':
                                valid_cam[b, cls_idx] = aux_mask[0, 0] * valid_cam[b, cls_idx]

                    else:
                        if isinstance(seg_t, float):
                            valid_cam[b, cls_idx][mask] = seg_t * auxiliaryMask[idx][0,mask] + (1-seg_t) * valid_cam[b, cls_idx][mask]
                        elif isinstance(seg_t, str):
                            if seg_t == 'max':
                                valid_cam[b, cls_idx][mask] = torch.max(auxiliaryMask[idx][0,mask] ,valid_cam[b, cls_idx][mask])
                            elif seg_t == 'x':
                                valid_cam[b, cls_idx][mask] =auxiliaryMask[idx][0,mask] * valid_cam[b, cls_idx][mask]



    cam_value, _pseudo_label = valid_cam.max(dim=1, keepdim=False)
    _pseudo_label += 1
    _pseudo_label[cam_value<=bkg_thre] = 0

    if ret:
        return valid_cam, _pseudo_label
    if img_box is None:
        return _pseudo_label

    if ignore_mid:
        _pseudo_label[cam_value<=high_thre] = ignore_index
        _pseudo_label[cam_value<=low_thre] = 0
    pseudo_label = torch.ones_like(_pseudo_label) * ignore_index

    for idx, coord in enumerate(img_box):
        pseudo_label[idx, coord[0]:coord[1], coord[2]:coord[3]] = _pseudo_label[idx, coord[0]:coord[1], coord[2]:coord[3]]

    return valid_cam, pseudo_label

def cam_to_roi_mask(cam, cls_label, img_box=None, bkg_thre=None, high_thre=None, low_thre=None, ignore_mid=False, ignore_index=None):
    b, c, h, w = cam.shape
    #pseudo_label = torch.zeros((b,h,w))
    cls_label_rep = cls_label.unsqueeze(-1).unsqueeze(-1).repeat([1,1,h,w])
    valid_cam = cls_label_rep * cam
    cam_value, _pseudo_label = valid_cam.max(dim=1, keepdim=False)
    _pseudo_label += 1
    _pseudo_label[cam_value<=bkg_thre] = 0

    if img_box is None:
        return _pseudo_label

    if ignore_mid:
        _pseudo_label[cam_value<=high_thre] = -1
        _pseudo_label[cam_value<=low_thre] = 0
    roi_mask = torch.ones_like(_pseudo_label) * ignore_index

    for idx, coord in enumerate(img_box):
        roi_mask[idx, coord[0]:coord[1], coord[2]:coord[3]] = _pseudo_label[idx, coord[0]:coord[1], coord[2]:coord[3]]

    return roi_mask

def ignore_img_box(label, img_box, ignore_index):

    pseudo_label = torch.ones_like(label) * ignore_index

    for idx, coord in enumerate(img_box):
        pseudo_label[idx, coord[0]:coord[1], coord[2]:coord[3]] = label[idx, coord[0]:coord[1], coord[2]:coord[3]]

    return pseudo_label
class OutputTarget:
    def __init__(self, category):
        self.category = category
    def __call__(self, model_output):
        if len(model_output.shape) == 1:
            return model_output[self.category]
        return model_output[:, self.category]
def multi_scale_cam2_v2(model, inputs, scales,cls_label=None, args=None,mask_circle = False,ignore_mask = False,grad_cam = None):
    '''process cam and aux-cam'''
    # cam_list, tscam_list = [], []
    
    b, c, h, w = inputs.shape
    if cls_label is not None:
        m = cls_label.nonzero().shape[0]
        repeat_label = cls_label.repeat(2,1)
    with torch.no_grad():
    
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0)

        penultimate_features,cls_token,_wsddn_seg,*_cam = model(inputs_cat, cam_only=True,cls_label=cls_label,args = args,mask_circle=mask_circle,ignore_mask = ignore_mask)
        if grad_cam is not None:
            _grad_cam = []
            with torch.enable_grad():
                for bb,cls_id in  repeat_label.nonzero():
                    _grad_cam.append(grad_cam(input_tensor=(penultimate_features[bb,None],_cam[0].shape[-2:],repeat_label[bb,None]),
                                                                                    targets=[OutputTarget(cls_id)],
                                                                                    target_size=None)[0].reshape(1,_cam[0].shape[-2],_cam[0].shape[-1]))
        _cam = [F.interpolate(i, size=(h,w), mode='bilinear', align_corners=False) for i in _cam]
        _cam = [F.relu(torch.max(i[:b,...], i[b:,...].flip(-1))) for i in _cam]
        cam_list = [[i] for i in _cam]
        if cls_label is not None:
            _wsddn_seg = torch.stack(_wsddn_seg)
            _wsddn_seg = F.interpolate(_wsddn_seg.unsqueeze(1), size=(h,w), mode='bilinear', align_corners=False)
            _wsddn_seg = torch.max(_wsddn_seg[:m,...],_wsddn_seg[m:,...].flip(-1)) #TODO 可以用mean试试
            wsddn_seg_list = [_wsddn_seg]
        if grad_cam is not None:
            _grad_cam = [torch.from_numpy(x).to(inputs.device) if isinstance(x, np.ndarray) else x for x in _grad_cam]
            _grad_cam = torch.stack(_grad_cam)
            _grad_cam = F.interpolate(_grad_cam, size=(h,w), mode='bilinear', align_corners=False)
            _grad_cam = torch.max(_grad_cam[:m,...],_grad_cam[m:,...].flip(-1)) #TODO 可以用mean试试
            grad_cam_list = [_grad_cam]



        for s in scales:
            if s != 1.0:
                _inputs = F.interpolate(inputs, size=(int(s*h), int(s*w)), mode='bilinear', align_corners=False)
                inputs_cat = torch.cat([_inputs, _inputs.flip(-1)], dim=0)

                penultimate_features,cls_token,_wsddn_seg,*_cam = model(inputs_cat, cam_only=True,cls_label=cls_label,args = args,ignore_mask = ignore_mask)
                if grad_cam is not None:
                    _grad_cam = []
                    with torch.enable_grad():
                        for bb,cls_id in  repeat_label.nonzero():
                            _grad_cam.append(grad_cam(input_tensor=(penultimate_features[bb,None],_cam[0].shape[-2:],repeat_label[bb,None]),
                                                                                            targets=[OutputTarget(cls_id)],
                                                                                    target_size=None)[0].reshape(1,_cam[0].shape[-2],_cam[0].shape[-1]))
                    _grad_cam = [torch.from_numpy(x).to(inputs.device) if isinstance(x, np.ndarray) else x for x in _grad_cam]
                    _grad_cam = torch.stack(_grad_cam)
                    _grad_cam = F.interpolate(_grad_cam, size=(h,w), mode='bilinear', align_corners=False)
                    _grad_cam = torch.max(_grad_cam[:m,...],_grad_cam[m:,...].flip(-1)) #TODO 可以用mean试试
                    grad_cam_list.append(_grad_cam)

                _cam = [F.interpolate(i, size=(h,w), mode='bilinear', align_corners=False) for i in _cam]
                _cam = [F.relu(torch.max(i[:b,...], i[b:,...].flip(-1))) for i in _cam]
                if cls_label is not None:
                    _wsddn_seg = torch.stack(_wsddn_seg)
                    _wsddn_seg = torch.max(_wsddn_seg[:m,...],_wsddn_seg[m:,...].flip(-1)) #TODO 可以用mean试试
                    _wsddn_seg = F.interpolate(_wsddn_seg.unsqueeze(1), size=(h,w), mode='bilinear', align_corners=False)
                    wsddn_seg_list.append(_wsddn_seg)

                for cam_i in range(len(cam_list)):
                    cam_list[cam_i].append(_cam[cam_i])


        cam = [torch.sum(torch.stack(i, dim=0), dim=0) for i in cam_list]
        cam = [i + F.adaptive_max_pool2d(-i, (1, 1)) for i in cam]
        cam = [i / (F.adaptive_max_pool2d(i, (1, 1)) + 1e-5) for i in cam]
        wsddn_seg = None
        grad_cam_seg = None
        if cls_label is not None:
            wsddn_seg = torch.sum(torch.stack(wsddn_seg_list, dim=0), dim=0)
            wsddn_seg = wsddn_seg + F.adaptive_max_pool2d(-wsddn_seg, (1, 1))
            # print(wsddn_seg.sum(dim=(-2,-1)))
            wsddn_seg /= F.adaptive_max_pool2d(wsddn_seg, (1, 1)) + 1e-5
        if grad_cam is not None:
            grad_cam_seg = torch.sum(torch.stack(grad_cam_list, dim=0), dim=0)
            grad_cam_seg = grad_cam_seg + F.adaptive_max_pool2d(-grad_cam_seg, (1, 1))
            # print(wsddn_seg.sum(dim=(-2,-1)))
            grad_cam_seg /= F.adaptive_max_pool2d(grad_cam_seg, (1, 1)) + 1e-5

    return wsddn_seg,grad_cam_seg,cam
def multi_scale_cam2(model, inputs, scales, cls_label=None,args = None):
    '''process cam and aux-cam'''
    # cam_list, tscam_list = [], []
    b, c, h, w = inputs.shape
    if cls_label is not None:
        m = cls_label.nonzero().shape[0]
    with torch.no_grad():
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0)
        if cls_label is not None:
            # s = time.time()
            _cam_aux, _cam, _wsddn_seg = model(inputs_cat, cam_only=True, cls_label=cls_label,args = args)
            # e = time.time()
            # print("in time1:",e-s)
            # _cam_aux, _cam, _wsddn_seg = model(inputs_cat, cam_only=True, cls_label=cls_label,args = args)
        else:
            _cam_aux, _cam = model(inputs_cat, cam_only=True)
        _cam = F.interpolate(_cam, size=(h,w), mode='bilinear', align_corners=False)
        _cam = torch.max(_cam[:b,...], _cam[b:,...].flip(-1))
        _cam_aux = F.interpolate(_cam_aux, size=(h,w), mode='bilinear', align_corners=False)
        _cam_aux = torch.max(_cam_aux[:b,...], _cam_aux[b:,...].flip(-1))
        if cls_label is not None:
            _wsddn_seg = torch.stack(_wsddn_seg)
            _wsddn_seg = F.interpolate(_wsddn_seg.unsqueeze(1), size=(h,w), mode='bilinear', align_corners=False)
            _wsddn_seg = torch.max(_wsddn_seg[:m,...],_wsddn_seg[m:,...].flip(-1)) #TODO 可以用mean试试
            wsddn_seg_list = [_wsddn_seg]

        cam_list = [F.relu(_cam)]
        cam_aux_list = [F.relu(_cam_aux)]

        for s in scales:
            if s != 1.0:
                _inputs = F.interpolate(inputs, size=(int(s*h), int(s*w)), mode='bilinear', align_corners=False)
                inputs_cat = torch.cat([_inputs, _inputs.flip(-1)], dim=0)

                if cls_label is not None:
                    # s = time.time()
                    _cam_aux, _cam, _wsddn_seg = model(inputs_cat, cam_only=True, cls_label=cls_label,args = args)
                    # e = time.time()
                    # print("in time2:",e-s)
                else:
                    _cam_aux, _cam = model(inputs_cat, cam_only=True)

                _cam = F.interpolate(_cam, size=(h,w), mode='bilinear', align_corners=False)
                _cam = torch.max(_cam[:b,...], _cam[b:,...].flip(-1))
                _cam_aux = F.interpolate(_cam_aux, size=(h,w), mode='bilinear', align_corners=False)
                _cam_aux = torch.max(_cam_aux[:b,...], _cam_aux[b:,...].flip(-1))
                if cls_label is not None:
                    _wsddn_seg = torch.stack(_wsddn_seg)
                    _wsddn_seg = torch.max(_wsddn_seg[:m,...],_wsddn_seg[m:,...].flip(-1)) #TODO 可以用mean试试
                    _wsddn_seg = F.interpolate(_wsddn_seg.unsqueeze(1), size=(h,w), mode='bilinear', align_corners=False)
                    wsddn_seg_list.append(_wsddn_seg)


                cam_list.append(F.relu(_cam))
                cam_aux_list.append(F.relu(_cam_aux))

        cam = torch.sum(torch.stack(cam_list, dim=0), dim=0)
        cam = cam + F.adaptive_max_pool2d(-cam, (1, 1))
        cam /= F.adaptive_max_pool2d(cam, (1, 1)) + 1e-5

        cam_aux = torch.sum(torch.stack(cam_aux_list, dim=0), dim=0)
        cam_aux = cam_aux + F.adaptive_max_pool2d(-cam_aux, (1, 1))
        cam_aux /= F.adaptive_max_pool2d(cam_aux, (1, 1)) + 1e-5
        if cls_label is not None:
            wsddn_seg = torch.sum(torch.stack(wsddn_seg_list, dim=0), dim=0)
            wsddn_seg = wsddn_seg + F.adaptive_max_pool2d(-wsddn_seg, (1, 1))
            wsddn_seg /= F.adaptive_max_pool2d(wsddn_seg, (1, 1)) + 1e-5
            return cam, cam_aux, wsddn_seg
        return cam, cam_aux
def get_token_mask(pseudo_label_aux,num_classes,labels):
    batch_size,H,W = pseudo_label_aux.shape
    valid_mask = (pseudo_label_aux != 255)  # 大小为 (batch_size, H, W)

    # 将忽略的标签暂时设置为 0，以便进行 One-Hot 编码
    labels_for_one_hot = pseudo_label_aux.clone()
    labels_for_one_hot[~valid_mask] = 0  # 将 255 的位置设置为 0

    # 进行 One-Hot 编码
    one_hot = torch.nn.functional.one_hot(labels_for_one_hot, num_classes=num_classes)  # 大小为 (batch_size, H, W, num_classes)

    # 交换维度以得到 (batch_size, num_classes, H, W) 的张量
    one_hot = one_hot.permute(0, 3, 1, 2).long()

    # 在忽略的位置上，将 One-Hot 张量的值设置为 255
    valid_mask_expanded = valid_mask.unsqueeze(1).expand_as(one_hot)  # 大小为 (batch_size, num_classes, H, W)
    one_hot[~valid_mask_expanded] = 255

    # 处理 labels，将不存在的类别在整个图像上设置为 255
    # labels 的形状为 (batch_size, num_classes)
    # 将其扩展为 (batch_size, num_classes, 1, 1)
    labels_expanded = labels.view(batch_size, num_classes-1, 1, 1)

    # 扩展到 (batch_size, num_classes, H, W)
    labels_expanded = labels_expanded.expand(-1, -1, H, W)

    # 创建一个布尔掩码，表示类别是否存在
    # 类别不存在的位置为 True，存在的位置为 False
    absent_class_mask = (labels_expanded == 0)  # 大小为 (batch_size, num_classes, H, W)
    one_hot = one_hot[:,1:]
    # 在对应位置上，将 One-Hot 张量设置为 255
    one_hot[absent_class_mask] = 255

    # 最终的标签张量
    label_20 = one_hot  # 大小为 (batch_size, 20, H, W)
    return one_hot

def label_to_aff_mask(cam_label, ignore_index=255):
    
    b,h,w = cam_label.shape

    _cam_label = cam_label.reshape(b, 1, -1)
    _cam_label_rep = _cam_label.repeat([1, _cam_label.shape[-1], 1])
    _cam_label_rep_t = _cam_label_rep.permute(0,2,1)
    aff_label = (_cam_label_rep == _cam_label_rep_t).type(torch.long)
    
    for i in range(b):
        aff_label[i, :, _cam_label_rep[i, 0, :]==ignore_index] = ignore_index
        aff_label[i, _cam_label_rep[i, 0, :]==ignore_index, :] = ignore_index
    aff_label[:, range(h*w), range(h*w)] = ignore_index
    return aff_label


def refine_cams_with_bkg_v2(ref_mod=None, images=None, cams=None, cls_labels=None, high_thre=None, low_thre=None, ignore_index=False,  img_box=None, down_scale=2, gumbel_sofmax = False):

    b,_,h,w = images.shape
    _images = F.interpolate(images, size=[h//down_scale, w//down_scale], mode="bilinear", align_corners=False)

    bkg_h = torch.ones(size=(b,1,h,w))*high_thre
    bkg_h = bkg_h.to(cams.device)
    bkg_l = torch.ones(size=(b,1,h,w))*low_thre
    bkg_l = bkg_l.to(cams.device)

    bkg_cls = torch.ones(size=(b,1))
    bkg_cls = bkg_cls.to(cams.device)
    cls_labels = torch.cat((bkg_cls, cls_labels), dim=1)

    refined_label = torch.ones(size=(b, h, w)) * ignore_index
    refined_label = refined_label.to(cams.device)
    refined_label_h = refined_label.clone()
    refined_label_l = refined_label.clone()
    
    cams_with_bkg_h = torch.cat((bkg_h, cams), dim=1)
    _cams_with_bkg_h = F.interpolate(cams_with_bkg_h, size=[h//down_scale, w//down_scale], mode="bilinear", align_corners=False)#.softmax(dim=1)
    cams_with_bkg_l = torch.cat((bkg_l, cams), dim=1)
    _cams_with_bkg_l = F.interpolate(cams_with_bkg_l, size=[h//down_scale, w//down_scale], mode="bilinear", align_corners=False)#.softmax(dim=1)

    for idx, coord in enumerate(img_box):

        valid_key = torch.nonzero(cls_labels[idx,...])[:,0]
        valid_cams_h = _cams_with_bkg_h[idx, valid_key, ...].unsqueeze(0).softmax(dim=1)
        valid_cams_l = _cams_with_bkg_l[idx, valid_key, ...].unsqueeze(0).softmax(dim=1)

        _refined_label_h = _refine_cams(ref_mod=ref_mod, images=_images[[idx],...], cams=valid_cams_h, valid_key=valid_key, orig_size=(h, w))
        _refined_label_l = _refine_cams(ref_mod=ref_mod, images=_images[[idx],...], cams=valid_cams_l, valid_key=valid_key, orig_size=(h, w))
        
        refined_label_h[idx, coord[0]:coord[1], coord[2]:coord[3]] = _refined_label_h[0, coord[0]:coord[1], coord[2]:coord[3]]
        refined_label_l[idx, coord[0]:coord[1], coord[2]:coord[3]] = _refined_label_l[0, coord[0]:coord[1], coord[2]:coord[3]]

    refined_label = refined_label_h.clone()
    refined_label[refined_label_h == 0] = ignore_index
    refined_label[(refined_label_h + refined_label_l) == 0] = 0

    return refined_label

def _refine_cams(ref_mod, images, cams, valid_key, orig_size):

    refined_cams = ref_mod(images, cams)
    refined_cams = F.interpolate(refined_cams, size=orig_size, mode="bilinear", align_corners=False)
    refined_label = refined_cams.argmax(dim=1)
    refined_label = valid_key[refined_label]

    return refined_label
