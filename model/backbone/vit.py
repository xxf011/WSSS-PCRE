...
class Attention(nn.Module):
    ...
    def forward(self, x,atten_mask = None):
        ...
        if atten_mask is not None:
            # Assuming attn_mask is a boolean tensor of shape (B, N, N),
            # where True values indicate positions to be masked
            atten_mask = atten_mask.unsqueeze(1)  # Shape becomes (B, 1, N, N) to broadcast across heads
            attn = attn.masked_fill(atten_mask, float('-inf'))
        # if patch_mask is not None:
        #     _attn = (q @ k.transpose(-2, -1)) * self.scale
        #     attn = _attn + patch_mask.unsqueeze(1)*5
        # else:
        weights =  F.normalize(q, dim=-1) @ F.normalize(k.transpose(-2, -1), dim=-2)
...
class Block(nn.Module):
    ...
    def forward(self, x,atten_mask = None,qkv = False):
        # qkv = True
        if qkv:
            y = self.norm1(x)
            y = F.linear(y, self.attn.qkv.weight, self.attn.qkv.bias)
            N, L, C = y.shape
            ...
            o, weights = self.attn(self.norm1(x),atten_mask = atten_mask)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # self.after_load_state_dict()
        self.apply(self._init_weights)
    def after_load_state_dict(self,):
        self.cls_token = nn.Parameter(self.cls_token.repeat(1, 20, 1))
        cls_token_embed = self.pos_embed[:, 0, :].unsqueeze(1).repeat(1, 20, 1)
        # trunc_normal_(cls_token_embed, std=.02)
        self.pos_embed = nn.Parameter(torch.cat([cls_token_embed,self.pos_embed[:,1:]], dim=1))
        # trunc_normal_(self.pos_embed, std=.02)
        # trunc_normal_(self.cls_token, std=.02)
    ...
    def prepare_tokens(self, x, num_cls = 1):
        B, nc, h, w = x.shape
        h, w = h // self.patch_embed.patch_size[0], w // self.patch_embed.patch_size[1]
        x = self.patch_embed(x)  # patch linear embedding

        patch_pos_embed = self.pos_embed[:, num_cls:, :].reshape(1, self._size, self._size, -1).permute(0, 3, 1, 2)
        ...
        pos_embed = torch.cat((self.pos_embed[:, :num_cls, :], patch_pos_embed), dim=1)
    ...
    def forward_features(self, x,atten_mask = None,ret_penultimate_features = False):
        ...
        # x = x + self.pos_embed
        num_cls = 20
        x = self.prepare_tokens(x,num_cls)
        penultimate_features = None
        x = self.pos_drop(x)
        embeds = []
        weight_list = None
        M = x.shape[1]
        atten_mask = torch.zeros(M, M).to(x.device)

        # 图像特征之间不掩蔽
        # cls_token之间不掩蔽，cls_token可以注意图像特征
        atten_mask[:num_cls, :num_cls] = 1  # cls_token之间掩蔽
        atten_mask[num_cls:, :num_cls] = 1  # 图像特征不能注意cls_token
        atten_mask[:num_cls, num_cls:] = 0  # cls_token可以注意图像特征
        atten_mask[num_cls:, num_cls:] = 0  # 图像特征之间不掩蔽
        ...
        for idx,blk in enumerate(self.blocks):
            if idx == len(self.blocks) - 1:
                ...
                qkv = False
            if ret_penultimate_features:
                if idx == self.depth-1:
                    penultimate_features = x
                    
                    
            x, weights = blk(x,atten_mask = atten_mask.unsqueeze(0), qkv = qkv)
            embeds.append(x)
            weight_list = weights

        x = self.norm(x)
        embeds[-1] = x
        return x[:, :num_cls], x[:, num_cls:], embeds[self.aux_layer][:, num_cls:],weight_list,penultimate_features
