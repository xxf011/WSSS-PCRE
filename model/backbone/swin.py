...
def get_block_by_idx(block_depths, idx):
    current_idx = 0
    for block_num, depth in enumerate(block_depths):
        if current_idx <= idx < current_idx + depth:
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, out_idx = None):
        """Forward function.
        Args:
            x: Input feature, tensor size (B, H*W, C).
            H, W: Spatial resolution of the input feature.
        ...
        for idx,blk in enumerate(self.blocks):
            ...
            if self.use_checkpoint:
                ...
                x = blk(x, attn_mask)
                if out_idx is not None and out_idx == idx:
                    x_out = x
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.patch_size = self._size_devisibility
        self.depths = depths
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            if i == 2 and self.aux_layer is not None:
                out_idx = self.aux_layer
            else:
                out_idx = None
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww, out_idx)
        ...
        return None, outs["p3"],outs["p2"],None
    def forward_features(self,x):
        return self.forward(x)

def swin_base_win12_384(pretrained=False, aux_layer = None,**kwargs):
    print(f"aux layer:  Block 2_{aux_layer}")
    ...
    if pretrained: 
        checkpoint = torch.load("pretrained/checkpoints/swin_base_patch4_window7_224_22k.pth", map_location="cpu")
